...
1.  Using the religion_overall dataframe, create a bar chart (`geom_col` or `geom_bar(stat="identity"))` showing the share of each religion_label.
2.  Map religion_label to the x-axis and share to the y-axis.
3.  Add appropriate labels using `labs()` for the title, x-axis, and y-axis.
4.  Format the y-axis to display percentages using scale_y_continuous`(labels = scales::label_percent())`.
5.  Apply a theme, for example `theme_minimal()`.
6.  Change the colour palette using `scale_fill_brewer()` (you'll need to map religion_label to the fill aesthetic as well).
...
1.  Using the working_hours_mean_adult dataframe created in step 10, create a grouped bar chart (`geom_col`) showing the mean_hours_worked_if_work (average hours for those who work).
2.  Map area_type (Rural/Urban) to the x-axis.
3.  Map mean_hours_worked_if_work to the y-axis.
4.  Map sex_label (Male/Female) to the fill aesthetic to get different coloured bars for men and women.
...
Finally, practice saving one of the plots you created. Assign one of your plots to a variable (e.g., migration_plot \<- ggplot(...) + ...). Use `ggsave()` to save this plot to a file. Include today's date in the filename using `Sys.Date()`. For example, save it as *"migration_plot_YYYY-MM-DD.png"*.
